...
extern "C" {
#endif
static char *classLinker = nullptr;
typedef void (*InitClassFunc)(void *, void *, int);
static InitClassFunc MakeInitializedClassesVisiblyInitialized = nullptr;
...
static int findInitClassSymbols(JNIEnv *env) {
    ...
    if(handle == nullptr) {
        ...
        if(!runtime) {
            LOGE("Runtime::instance value is nullptr");
            return 1;
        }
...
jlong __attribute__((naked)) getThread(JNIEnv *env, jclass clazz) {
#if defined(__aarch64__)
    __asm__(
            "mov x0, x19\n"
            "ret\n"
...
jint visiblyInit(JNIEnv *env, jclass clazz, jlong thread) {
    ...
    LOGI("thread is at %p", (void*)thread);
