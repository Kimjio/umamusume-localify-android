...
void *genTrampoline(void *toMethod, void *entrypoint) {
    size_t trampolineSize = entrypoint != nullptr ? sizeof(trampolineForBackup) : sizeof(trampoline);

    // check available space for new trampoline
    if(currentTrampolineOff+trampolineSize > trampolineSpaceEnd) {
        currentTrampolineOff = (u_char *)allocTrampolineSpace();
        if (currentTrampolineOff == nullptr) {
            return nullptr;
        } else {
...
static void *allocTrampolineSpace() {
    auto *buf = (u_char*)mmap(nullptr, TRAMPOLINE_SPACE_SIZE, PROT_READ | PROT_WRITE | PROT_EXEC,
